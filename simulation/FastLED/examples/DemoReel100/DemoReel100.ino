...
void setup() {
  ...
  FastLED.addLeds<NEOPIXEL, 1>(leds[0], NUM_LEDS);
    FastLED.addLeds<NEOPIXEL, 2>(leds[1], NUM_LEDS);
    FastLED.addLeds<NEOPIXEL, 3>(leds[2], NUM_LEDS);
    FastLED.addLeds<NEOPIXEL, 4>(leds[3], NUM_LEDS);
...
void juggle() {
  // eight colored dots, weaving in and out of sync with each other
    for(int j = 0; j < 4; j++){
  fadeToBlackBy( leds[j], NUM_LEDS, 20);
  uint8_t dothue = 0;
